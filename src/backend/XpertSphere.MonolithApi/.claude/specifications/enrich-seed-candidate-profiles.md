...
Ce correctif de seed **ne corrige pas** ce problème (il relève de `recruiter-app`, un package distinct avec son propre `.claude/specifications/`, et modifier une interface TypeScript + un template Vue dépasse le périmètre d'une spec de données de seed backend). L'agent principal doit décider s'il souhaite qu'une spec séparée soit rédigée côté `recruiter-app` (ex. `fix-experience-fields-candidate-detail-page.md`) avant la démonstration prévue par l'utilisateur, pour que la section "Expériences professionnelles" s'affiche complètement.
...
- **Le signalement de l'incohérence `recruiter-app`/`Experience`** (§Hors périmètre) n'est pas une question bloquante pour ce correctif-ci (qui reste backend/données), mais nécessite une décision de l'agent principal : ouvrir ou non une spec `recruiter-app` séparée avant la démonstration.
