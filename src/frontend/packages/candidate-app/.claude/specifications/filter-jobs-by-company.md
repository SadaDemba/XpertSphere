...
- Une nouvelle action, par exemple `fetchCompanyFilterOptions(): Promise<void>` :
  ...
  - en cas d'échec (réponse non `isSuccess`, ou exception), fixer `companyFilterSourceOffers.value = []` (dégrade vers une dropdown vide/désactivée, sans erreur bloquante).
...
1. Remplacer `class="row q-gutter-md"` par `class="row q-col-gutter-md"` sur la ligne des filtres.
