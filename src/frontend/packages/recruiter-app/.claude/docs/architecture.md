# Architecture détaillée — recruiter-app (XpertSphere)

Ce document rassemble le détail technique de `@xpertsphere/recruiter-app` :
stack, structure du projet, conventions de code et intégration avec le
backend. Le fichier `CLAUDE.md` à la racine du package reste volontairement
court et renvoie ici pour le détail.
