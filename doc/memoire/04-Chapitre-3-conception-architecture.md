...
Derrière la passerelle, le cœur applicatif repose sur une *API Monolith .NET* entourée de services spécialisés : un *service d'intégration externe* prévu, dans une perspective d'industrialisation, pour connecter la plateforme à des sources d'offres tierces (HelloWork, LinkedIn Jobs, Welcome to the Jungle) et à des outils de planification comme Google Calendar, un *service de reporting* interfacé avec Power BI, un *analyseur de CV* en FastAPI s'appuyant sur Azure OpenAI, et un *service de communication* pour les échanges sortants.
...
En dehors du noyau monolithique, plusieurs services spécialisés existent dans la structure du projet. Le *service d'intégration externe*, développé en .NET, a vocation, dans une perspective d'industrialisation, à assurer la synchronisation avec des plateformes tierces d'annonces d'emploi (HelloWork, LinkedIn Jobs, Welcome to the Jungle) ainsi qu'avec des outils de planification comme Google Calendar, afin de relier XpertSphere à l'écosystème numérique des recruteurs. Ce module est posé dans l'architecture comme axe d'extension prévu ; son déploiement effectif reste à venir. Le *service de reporting*, également en .NET, produit les analyses de KPI et s'interface avec Power BI pour les tableaux de bord avancés. L'*analyseur de CV*, implémenté en FastAPI, s'appuie sur Azure OpenAI pour l'extraction sémantique des compétences et des expériences professionnelles. Enfin, le *service de communication* orchestre l'ensemble des échanges sortants à destination des utilisateurs. Ces modules restent indépendants du cœur applicatif et peuvent évoluer à leur propre rythme.
...
Pour l'environnement cible sur Azure, l'architecture s'appuie sur des services managés pour réduire la charge d'exploitation. Les services applicatifs — API monolithique et services satellites — sont déployés sur *Azure Container Apps* (ACA), à partir d'images construites et versionnées dans *Azure Container Registry* (ACR), ce qui est cohérent avec la logique de conteneurisation illustrée dans la Figure 3.1. Les bases de données sont confiées à Azure SQL Database, qui gère nativement les sauvegardes et la haute disponibilité. Les assets (images, CV) sont stockés sur Azure Blob Storage, accessible via un CDN pour optimiser les performances d'accès au Sénégal et au-delà.
